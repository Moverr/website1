  ...
  cc.Popup = (function() {
    ...
    var defaultOptions = {
      ...
      elements: {
        ...
        // dismiss: '<a aria-label="dismiss cookie message" tabindex="0" class="cc-btn cc-dismiss"><i class="fa fa-close"></i></a>',
        dismiss: '<button aria-label="dismiss cookie message" tabindex="0" class="cc-btn cc-dismiss">X</button>',
      ...
      if (util.hasClass(el, 'cc-invisible')) {
        ...
        if (this.options.static) {
          ...
          if(window.matchMedia('(max-width: 768px)').matches) {
            ...
            $(".banner.big-bg").css("height", "-=" + height);
          }

          // $("header").css("margin-top", height);
          // bottom = $("header").position().top + $("header").outerHeight(true);
          // $(".sub_header").css("top", bottom);
        }
...
function startPage() {
    // passar inicializacoes para aqui
}
...
$(function(){
    ...
    $(".chartjs").map(function(index, item) {
        // $item = $(item);
        graph_class = $(item).data('class');

        if (graph_class === undefined) {
            graph_class = 'graph-default';
        }

        graph_data[index] = {
            graph_id: $(item).data("id"),
            class: graph_class,
            item: $(item)
        }

        if (!graph_data[index].graph_id) {
            return;
        }
        ...
        $.ajax({
            url: '/getGraph',
            type: "GET",
            data: {id: graph_data[index].graph_id},
            success: function(result) {
                if (result) {
                    graph_data[index].item.append("<canvas id='graph_"+graph_data[index].graph_id+"' class='"+graph_data[index].class+"'></canvas>");
                    canvas = $("#graph_" + graph_data[index].graph_id);

                    drawGraph(canvas, result);
                }
                else {
                    graph_data[index].item.hide().remmove();
                }
            },
            error: function() {
                graph_data[index].item.hide().remove();
            },
            dataType: "json",
        });
    });
...
function drawGraph(canvas, result) {
    var prefix = "", suffix = "", yAxes = "", xAxes = "";

    if (result.data.prefix)
        prefix = result.data.prefix;
    if (result.data.suffix)
        suffix = result.data.suffix;

    if (result.data.title_yAxes)
        yAxes = result.data.title_yAxes;

    if (result.data.title_xAxes)
        xAxes = result.data.title_xAxes;
    ...
    datasets = [];
    $.each(result.data.dataset, function(index, item) {
        dataset = {
            label: item.legend,
            borderWidth: 1
        };
        ...
        if (result.type == "line" || result.type == "bar") {
            dataset.backgroundColor = "#" + (item.color ? item.color : "c8c8c8");
        }
        else {
            backgroundColor = [];
            $.each(result.data.data, function(child_index, child_item) {
                backgroundColor.push("#" + (child_item.color ? child_item.color : "c8c8c8"));
            });
        ...
        $.each(result.data.data, function(child_index, child_item) {
            data.push(child_item.values[index]);
        });
    ...
    $("<div style='clear:both'></div>").insertAfter(canvas.parent());
}
...
$("body, window").on("click", ".gal-video, .video-container", function(e) {
    e.preventDefault();
    ...
    if (id && src) {
        ...
        if (btn.hasClass('is-playing')) {
            ...
            $(".play-btn").removeClass('is-playing');
            playPause.removeClass("play").addClass("pause");
                      
            $(".video-thumb").show();
            btn.siblings(".video-thumb").hide();
